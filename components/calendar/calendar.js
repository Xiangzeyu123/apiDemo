  ...
  methods: {
    ...
    setSpot() {
      ...
      const timeArr = this.data.spot.map(item => {
        return this.formatTime(item.day, "Y-M-D")
      })
    ...
    selectChange(e) {
      ...
      // this.triggerEvent("tabsDateChange",{selectDay})
      this.triggerEvent("kurikku",selectDay)
      if (this.data.selectDay.year !== year || this.data.selectDay.month !== month) {
        ...
        this.triggerEvent("kurikku", this.data.selectDay)
      }
    }
  },
  lifetimes: {
