...
function addSheet(workCase,workbook,dateInfo){
    ...
    for(let i=0;i<workCase.userInfo.length;i++){
        ...
        cellA13Next.value=workCase.userInfo[i].nickName
        //设置时间和分钟
    ...
    for(let i =0;i<workCase.userInfo.length;i++){//名字
        if(workCase.userInfo[i].userWorkInfo.length==0){
            ...
            for(let w=0;w<workCase.userInfo[i].userWorkInfo.length;w++){
                ...
                workSheet.getCell(cellC13.row+i*2+1,cellC13.col+today).value=workCase.userInfo[i].userWorkInfo[w].hour
                workSheet.getCell(cellC13.row+i*2+2,cellC13.col+today).value=workCase.userInfo[i].userWorkInfo[w].minute
            }
        }
    }
    return workbook
