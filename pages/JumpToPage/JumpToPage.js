...
Page({
  data: {
    ...
    contractNumber: null,
    Director: null,
    contractSubject: {name:null,index:null,id:null},
    supervisor: null,
    workStatus: "",
    showDatetime: "",
    show: false,
  ...
  dateTap(even) {
    ...
    workDataList.forEach((item) => {
      if (item.day == even.detail.dateString) {
        console.log(item.endTime,item.beginTime)
        this.setData({
          closingTtime: item.endTime,
          workHours: item.beginTime,
          selectTime: item.day,
          showFrom: true,
          'contractSubject.name':item.caseData.caseName,
          'contractSubject.id':item.caseData.caseId,
          workStatus: {
            value: columns[item.status],
            index: item.status
          }
        })
      }
